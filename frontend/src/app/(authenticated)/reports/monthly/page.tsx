    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={handleExport} disabled={!wr || activeTab === 'summary'} className="flex items-center gap-1.5 px-3 py-2 text-sm text-gray-600 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors disabled:opacity-40">
            ...
            {[-2, -1, 0].map((offset) => {
              ...
              return (
                <button
                  ...
                  className="px-3 py-1.5 text-xs border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-600 dark:text-gray-300"
                >
                  {format(d, 'MMM yyyy')}
                </button>
              );
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: 'Newly Added', value: wr.newlyAdded?.length ?? 0, color: 'text-green-600', bg: 'bg-green-50 dark:bg-green-900/40', icon: Plus },
              { label: 'Closed', value: wr.closedThisWeek?.length ?? 0, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-900/40', icon: X },
              { label: 'Changed', value: wr.changedThisWeek?.length ?? 0, color: 'text-orange-600', bg: 'bg-orange-50 dark:bg-orange-900/40', icon: RefreshCw },
              { label: 'Managers Active', value: (new Set(wr.changedThisWeek?.map((p: any) => p.projectManager) || [])).size, color: 'text-purple-600', bg: 'bg-purple-50 dark:bg-purple-900/40', icon: Users },
            ].map((item) => (
          ...
          <div className="flex bg-gray-100 dark:bg-gray-700 rounded-xl p-1 gap-1 w-fit">
            {tabs.map((tab) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === tab.id ? 'bg-white dark:bg-gray-800 text-primary-700 dark:text-primary-400 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}
              >
                {tab.label}
              </button>
            ))}
          ...
          <Card>
            {activeTab === 'summary' && (
              <div className="space-y-4">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white">Monthly Summary</h3>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                  <div className="p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                    <p className="text-gray-500 dark:text-gray-400 text-xs mb-1">New projects added</p>
                    <p className="text-xl font-bold text-green-600">{wr.newlyAdded?.length ?? 0}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                    <p className="text-gray-500 dark:text-gray-400 text-xs mb-1">Projects closed / cancelled</p>
                    <p className="text-xl font-bold text-blue-600">{wr.closedThisWeek?.length ?? 0}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                    <p className="text-gray-500 dark:text-gray-400 text-xs mb-1">Projects with changes</p>
                ...
                {wr.summary?.byManager && wr.summary.byManager.length > 0 && (
                  <div>
                    <h4 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">By Manager</h4>
                    <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700">
                      <table className="w-full text-sm">
                        <thead className="bg-gray-50 dark:bg-gray-700/50">
                          <tr>
                            {['Manager', 'Added', 'Closed', 'Changed'].map((h) => (
                              <th key={h} className={`py-2 px-3 text-xs font-medium text-gray-500 dark:text-gray-400 ${h === 'Manager' ? 'text-left' : 'text-center'}`}>{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {wr.summary.byManager.map((m: any) => (
                            <tr key={m.manager} className="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                              <td className="py-2 px-3 font-medium text-gray-800 dark:text-gray-200">{m.manager}</td>
            ...
            {activeTab === 'added' && (
              <div>
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">Newly Added Projects</h3>
                      ...
                      <thead className="bg-gray-50 dark:bg-gray-700/50">
                        <tr>
                          {['Project Name', 'Customer', 'Manager', 'Migration Type', 'Added On'].map((h) => (
                            <th key={h} className={`py-2 px-3 text-xs font-medium text-gray-500 dark:text-gray-400 ${h === 'Project Name' ? 'text-left' : 'text-center'}`}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {wr.newlyAdded.map((p: any) => (
                          <tr key={p.id} className="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                            ...
                            <td className="py-2.5 px-3 text-center text-gray-600 dark:text-gray-400">{p.customerName}</td>
                            <td className="py-2.5 px-3 text-center text-gray-600 dark:text-gray-400">{p.projectManager}</td>
            ...
            {activeTab === 'closed' && (
              <div>
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">Closed / Cancelled Projects</h3>
            ...
            {activeTab === 'changes' && (
              <div>
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">Projects with Changes</h3>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {wr.changedThisWeek.map((p: any, i: number) => (
                          <tr key={`${p.id}-${i}`} className="border-t border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                            <td className="py-2.5 px-3">
                              <Link href={`/projects/${p.id}`} className="font-medium text-primary-600 hover:underline">{p.name}</Link>
                            </td>
                            <td className="py-2.5 px-3 text-center text-gray-600 dark:text-gray-400">{p.customerName}</td>
